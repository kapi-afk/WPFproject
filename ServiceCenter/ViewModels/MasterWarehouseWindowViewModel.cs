...
namespace ServiceCenter.ViewModels
{
    public class MasterWarehouseWindowViewModel : BaseViewModel
    {
        ...
        private static string ValidationTitle => App.GetString("WarehouseValidationTitle", "Ошибка проверки");
        ...
        public bool CreateMaterialRequest()
        {
            ...
            {
                MessageBox.Show("Сначала выберите заявку в панели мастера.", ValidationTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }

            if (!int.TryParse(MaterialUsageQuantity, out var quantity) || quantity <= 0)
            {
                MessageBox.Show("Укажите корректное количество.", ValidationTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }
            ...
            {
                MessageBox.Show("Укажите, какой материал нужен.", ValidationTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }

            if (requestedName.Length > 120)
            {
                MessageBox.Show("Название материала не должно быть длиннее 120 символов.", ValidationTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }
            ...
            {
                MessageBox.Show("Категория не должна быть длиннее 80 символов.", ValidationTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return false;
            }
            ...
            MessageBox.Show("Заявка на материал отправлена администратору.", SuccessTitle, MessageBoxButton.OK, MessageBoxImage.Information);
            return true;
        }
        ...
        private void ConsumeWarehouseItem()
        {
            ...
            if (SelectedWarehouseItem == null)
            {
                MessageBox.Show("Выберите материал для списания.", ValidationTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            if (actualItem == null)
            {
                MessageBox.Show("Материал не найден на складе.", ErrorTitle, MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            MessageBox.Show($"Материал списан на заявку №{_targetOrder.Id}.", SuccessTitle, MessageBoxButton.OK, MessageBoxImage.Information);
        }
